...
from utils.nlm_dataloader import LoadNLMFeatures
from utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, \
    scale_coords, xyxy2xywh, strip_optimizer, set_logging, increment_path, save_one_box
from utils.nlm_plots import colors, plot_one_box
...
def detect(opt):
    ...
    # Run inference
    if device.type != 'cpu':
        model(torch.zeros(1, imgsz, imgsz, dataset.dimensions).to(device).type_as(next(model.parameters())))  # run once
    t0 = time.time()
